...
public class ThreadRun {
	...
	// 병렬 처리는 하나의 작업을 여러 부부으로 나누어서 동시에 처리하는 것.
...
//		thread2.run();
		...
		// start 나, stop 과 같은 Thread 메서드를 사용할 수 없다.
		// -> Thread 클래스에 Runnable 인터페이스를 상속받아 구현한
		// ThreadWithRunnable 객체를 전달하여 사용해야 함.
		
		thread2.start(); // 스레드의 실행
		
		// 스레드가 1초 간격으로 동시 실행되고 있는 것을 확인할 수 있음
		
		// 실무
		// - 웹서버, 데이터베이스처리, 대규모 데이터처리 등등
	}
